...
MotionAction::MotionAction(double x, double y, enum ActionId id) : BaseAction(x, y, 0, id)
{
}
...
ostream &operator<<(ostream &out, BaseAction &action)
{
	...
	<< ": [x: " << action.x << ", y: " << action.y << ", t: "
	<< action.t << ", c(x): " << action.cost << ", g(x): " << action.gcost << ", h(x): " << action.hcost << " }\n";
}